...
class ContextAdaptation
  ...
  def deploy
    if instance_adaptation?
      ...
      klass.define_singleton_method(self.method_name,self.implementation)
    end
  end

  # True if the adapted method is an instance method
  def instance_adaptation?
    instance_method?(klass,method_name)
  end

  #True if the adaptation concern the class n_klass and method n_method
  def concern(n_klass,n_method_name)
    klass==n_klass && method_name==n_method_name
  end
